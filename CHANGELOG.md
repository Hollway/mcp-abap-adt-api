...
### Новые инструменты — словарь, тексты, энхансменты
...
- **`createDataElement` и `createDomain`** создают объект словаря и сразу
  ...
  каком шаге остановились и в каком состоянии объект.
...
- **`transportDetails`** — что внутри запроса: его задачи и все объекты. Раньше
  ...
  `runQuery`.
- **`typeHierarchy`** — предки и потомки класса или интерфейса, **`whereUsedMethod`**
  — вызывающие метод. Оба принимают имя: бэкенд отвечает по позиции курсора, а
  считать строку и колонку в непрочитанном исходнике — именно то, из-за чего
  `usageReferences` оставался неиспользуемым.
...
- **`runTests`** — активировать, прогнать тесты и объяснить результат: сколько
  методов выполнилось, сколько прошло, и каждое падение с сообщением ABAP Unit.
...
### Исправления, найденные живыми прогонами новых инструментов
...
- **Объекты создаются на языке входа, а не на EN.** Библиотека подставляет в
  ...
  читался обратно с пустым описанием и четырьмя пустыми метками.
...
- **Дескриптор блокировки берётся из реестра.** `unLock`, `setObjectSource` и
  `createTestInclude` требовали его, хотя сервер его держит, — цепочку из трёх
  вызовов нельзя было провести, не перенося дескриптор руками.
...
- **404 от эндпоинтов доменов и текстовых элементов объясняется.** На этой
  системе `/sap/bc/adt/textelements` отсутствует целиком, а домены отвечают 404
  по любому пути, включая валидацию, — при этом элементы данных обслуживаются
  нормально, из-за чего пустой 404 читался как опечатка в имени.
...
- Появились модульные тесты (174) и сквозной smoke-скрипт только для чтения
  (`npm run smoke`); до этого `npm test` не находил ни одного теста.
- `deleteObject`, `unLock`, `setObjectSource` и `createTestInclude` принимают
  `lockHandle` необязательно — берут удерживаемый сервером.
